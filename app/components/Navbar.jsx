    ...
    <>
        ...
        <div className="max-w-4xl mx-auto px-6 flex items-center justify-center py-4">
          ...
          <ul className="hidden md:flex gap-2 md:gap-6 flex-nowrap text-sm font-medium tracking-wide whitespace-nowrap">
            {links.map(({ href, label }) => {
              ...
              return (
                <li key={href}>
                  <Link href={href} className={`px-2 md:px-4 py-2 rounded-full transition-colors duration-200 ${active ? "text-ink underline underline-offset-4 decoration-2" : "text-ink hover:text-sage"}`}>
                    {label}
                  </Link>
                </li>
              );
      ...
      >
        ...
        <button
          ...
          className="absolute top-4 right-6 p-2 rounded-md focus:outline-none"
        >
          <svg className="w-6 h-6 text-ivory" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              return (
                <li key={href}>
                  <Link
                    href={href}
                    onClick={close}
                    className={`block px-8 py-2 transition-colors duration-200 ${active ? "text-ivory underline underline-offset-4 decoration-2" : "text-ivory hover:text-sage"}`}
                  >
                    {label}
                  </Link>
                </li>
